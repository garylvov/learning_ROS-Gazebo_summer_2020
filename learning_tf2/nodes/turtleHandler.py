...
def checkAsterCollision(name):

    aster1Buff = tf2_ros.Buffer()

    listenerAster1 = tf2_ros.TransformListener(aster1Buff)

    turtleExists = True
    while turtleExists and not rospy.is_shutdown():
        try:
            aster = aster1Buff.lookup_transform(name, 'asteroid1', rospy.Time())
        except(tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
            continue
        if(.5 > math.sqrt(aster.transform.translation.x ** 2 + aster.transform.translation.y ** 2)):
            rospy.wait_for_service('kill')
            remove = rospy.ServiceProxy('kill', turtlesim.srv.Kill)
            remove(name)
            turtleExists = False
        aster = aster1Buff.lookup_transform(name, 'asteroid2', rospy.Time())
        if(.5 > math.sqrt(aster.transform.translation.x ** 2 + aster.transform.translation.y ** 2)):
            rospy.wait_for_service('kill')
            remove = rospy.ServiceProxy('kill', turtlesim.srv.Kill)
            remove(name)
            turtleExists = False
        aster = aster1Buff.lookup_transform(name, 'asteroid3', rospy.Time())
        if(.5 > math.sqrt(aster.transform.translation.x ** 2 + aster.transform.translation.y ** 2)):
            rospy.wait_for_service('kill')
            remove = rospy.ServiceProxy('kill', turtlesim.srv.Kill)
            remove(name)
            turtleExists = False
        aster = aster1Buff.lookup_transform(name, 'asteroid4', rospy.Time())
        if(.5 > math.sqrt(aster.transform.translation.x ** 2 + aster.transform.translation.y ** 2)):
            rospy.wait_for_service('kill')
            remove = rospy.ServiceProxy('kill', turtlesim.srv.Kill)
            remove(name)
...
if __name__ == '__main__':
    ...
    missile3.start()
    m1 = threading.Thread(target = checkAsterCollision, kwargs = dict(name = 'missile1'))
    m1.start()
    m2 = threading.Thread(target = checkAsterCollision, kwargs = dict(name = 'missile2'))
    m2.start()
    m3 = threading.Thread(target = checkAsterCollision, kwargs = dict(name = 'missile3'))
    m3.start()
